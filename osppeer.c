...
#define TASKBUFSIZ	32768	// Size of task_t::buf
				// Increased to connect to popular tracker
...
static void task_download(task_t *t, task_t *tracker_task)
{
	...
	// We're gonna see if those other kids fixed the filename vulnerability
	// Request a file with a name longer than FILENAMESIZ
	// Increase it by one character after every iteration
	int riri = 1;
	if (evil_mode)
	{
		char *attack;
		int len = 3 * FILENAMESIZ + riri, j;
		riri++;
		attack = (char *)malloc(sizeof(char) * len);
		for(j = 0; j < len; j++)
			attack[j] = 'A';
		osp2p_writef(t->peer_fd, "GET %s OSP@P\n", attack);
		free(attack);		
		goto try_again;
	}
	else
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
		...
		// Each buffer is loaded onto the disk
		// Cap how many continuous buffers can be loaded onto disk
		// This way peers can't fill up our disk
		if (t->total_written > TASKBUFSIZ * 10) 
			error("It's too big man! (The file size)");
			// Since the file is too big we probably don't want to try again
...
static void task_upload(task_t *t)
{
	...
	// Make sure the peers aren't trying to get at files in other directories
	char expBuf[PATH_MAX], reqBuf[PATH_MAX];
	char *expectedPath = getcwd(expBuf, PATH_MAX); // Expected Directory
	char *requestedPath = realpath(t->filename, reqBuf); // The file's actual path
	if (strncmp(expectedPath, requestedPath, strlen(expectedPath))) {
		error("Wants to upload a file from another directory! ALERT! ALERT!");
		goto exit;
	}
	
	// Let's see if those kids fixed the file size vulnerability
	// urandom is an infinitiely large file which will be sure to fill up
	// our victim's disk, eventually...
	if (evil_mode)
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
	...
	for (; argc > 1; argc--, argv++)
		if ((t = start_download(tracker_task, argv[1])))
		{
			pid_t dpid = fork();
			if (dpid == -1)
				error ("I can't make any children (to download)!");
			else if (dpid == 0)
			{
				task_download(t, tracker_task);
				_exit(0);
	...
	// Wait for all child processes to finish up
	while (dchild != 0)
		waitpid(-1, NULL, 0);
	...
	*/
	...
	// AND upload them in parallel too!
//	/*
	int uchild = 0;
	while ((t = task_listen(listen_task)))
	{
		pid_t upid = fork();
		waitpid(-1, NULL, WNOHANG); // Return immediately if no child exists
		if (upid == -1)
			error ("I can't make any children (to upload)!");
		else if (upid == 0)
		{
			task_upload(t);
			_exit(0);
